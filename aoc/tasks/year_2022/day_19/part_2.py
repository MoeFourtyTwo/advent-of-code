...
def go(path: pathlib.Path = DATA_PATH) -> int:
    ...
    blueprints = list(map(Blueprint.parse, lines))[:3]

    multiplied = functools.reduce(operator.mul, (blueprint.simulate(32) for blueprint in tqdm(blueprints)))

    logger.info(f"{multiplied=}")
