...
def go(path: pathlib.Path = DATA_PATH) -> int:
    ...
    for row_start in range(len(data)):
        max_energized_tiles = max(Solver.solve(data, (row_start, 0, 0, 1)), max_energized_tiles)
        max_energized_tiles = max(Solver.solve(data, (row_start, len(data[0]) - 1, 0, -1)), max_energized_tiles)

    for col_start in range(len(data[0])):
        max_energized_tiles = max(Solver.solve(data, (0, col_start, 1, 0)), max_energized_tiles)
        max_energized_tiles = max(Solver.solve(data, (len(data) - 1, col_start, -1, 0)), max_energized_tiles)
