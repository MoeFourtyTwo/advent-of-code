...
@functools.cache
def solve(search_string: str, available_patterns: frozenset[str], min_pattern_len: int, max_pattern_len: int) -> int:
    ...
    for split_len in range(min_pattern_len, min(len(search_string), max_pattern_len) + 1):
        ...
        if left in available_patterns and (count := solve(right, available_patterns, min_pattern_len, max_pattern_len)):
            solve_count += count
